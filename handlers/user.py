    ...
    try:
        ...
        if (message.text.lower() in ["да", "да!"]):
            ...
            if (prize is not None):
                ...
                # Проставляем в коллекции "Подарки" отметку, что этот подарок забрал текущий пользователь
                await db_service.mark_gift(db, prize, message.chat.id)
                # И отправляем поздравительное сообщение
